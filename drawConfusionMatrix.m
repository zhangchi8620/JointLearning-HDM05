...
set(t,'HorizontalAlignment','right','VerticalAlignment','top', 'Rotation',45, 'fontSize', 16);

t = text(Xt(1)*ones(1,length(Xt))-0.7,Yt,label);
set(t,'HorizontalAlignment','right','VerticalAlignment','middle', 'Rotation',0, 'fontSize', 16);

textalign = { 'VerticalAlignment','middle', 'HorizontalAlignment','center', 'color', [1 1 1] };
for i = 1:n_type
    for j = 1:n_type
        label = sprintf('%.3f',data(i,j));
